            ...
            case '1':
                ...
                if (todaysMessage) {
                    ...
                    if (todaysMessage.audio_url) {
                        ...
                        twiml.play(`${req.protocol}://${req.get('host')}/audio/${todaysMessage.recorded_audio}`);
                    } else {
    ...
    try {
        ...
        if (message.rows.length > 0) {
            ...
            if (msg.audio_url) {
                ...
                twiml.play(`${req.protocol}://${req.get('host')}/audio/${msg.recorded_audio}`);
            } else {
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .audio-controls { display: flex; align-items: center; gap: 1rem; margin-top: 1rem; }
            ...
            grid.innerHTML = currentMessages.map(msg => \`
                <div class="message-card">
                    ...
                    <p style="color: #666; margin: 0 0 1rem;">
                        📅 \${new Date(msg.date_recorded).toLocaleDateString()}
                    </p>
                    
                    \${msg.audio_url || msg.recorded_audio ? \`
                        <div class="audio-controls">
                            <audio controls style="width: 100%;">
                                <source src="\${msg.audio_url || '/audio/' + msg.recorded_audio}" type="audio/mpeg">
                            </audio>
                        </div>
                    \` : '<p style="color: #ff9500; font-weight: 500;">⚠️ No audio uploaded</p>'}
                    
                    <div class="audio-controls">
                        <button class="btn" onclick="editMessage(\${msg.day_number})" style="background: #34c759; font-size: 0.875rem; padding: 8px 16px;">
                            ✏️ Edit
                        </button>
                        <button class="btn danger" onclick="deleteMessage(\${msg.day_number})" style="font-size: 0.875rem; padding: 8px 16px;">
                            🗑️ Delete
                        </button>
                    </div>
                </div>
...
initDB().then(() => {
    ...
    app.listen(PORT, () => {
        console.log(\`🕊️ Nishmas IVR server running on port \${PORT}\`);
